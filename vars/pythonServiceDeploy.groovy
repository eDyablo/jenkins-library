          ...
          if (!config.skipConfiguring) {
            container('jenkins-slave') {
              sh '''
                ...
                echo "Installing configAndSecrets tools"
                set +x
                urlWithoutHttp=${NEXUS_PIP_REPO_URL#http://}
                baseurl=${urlWithoutHttp%%/*}
                fullUrlWithCreds="http://$NEXUS_USER:$NEXUS_PASSWORD@$urlWithoutHttp/simple"

                pip3 download configandsecrets -i $fullUrlWithCreds  --trusted-host $baseurl
                set -x
                ...
                do
                  ...
                  echo "Applying configMaps to $namespace"
                  python configandsecrets/create_config.py $AH_CONFIGURATION_PATH --map-name=$SERVICE_NAME_LOWER-$GITSCM_TAG-config --env-map-name=$SERVICE_NAME_LOWER-$GITSCM_TAG-env --env=$AH_CLUSTER $OVERRIDEN_CONFIGS | kubectl create --namespace=$namespace --filename=-
