...
class PipConfigTest {
  final config = new PipConfig()
  final pipeline = mock(DummyPipeline)

  @Test void config_dir_contains_the_class_name() {
    assertThat(config.configDir, containsString(PipConfig.name))
  }
  ...
  @Test void defines_env_var_pip_config_file_equals_to_its_config_path() {
    config.configRef.with {
      name = 'dir'
      key = 'file'
    }
    ...
    verify(pipeline).envVar(mapContains(
      key: PipConfig.CONFIG_FILE_ENV_VAR,
